...
const renderRows = () => {
  ...
  if (!state.readOnly) {
    ...
    if (appMode !== "wishlist") {
      ...
      saleBtn.textContent = isForSale(row) ? "For Sale" : "For Sale";
...
const renderAdminStats = (data) => {
  if (!adminStatsContent) return;
  adminStatsContent.textContent = "";
  ...
  adminStatsContent.appendChild(summary);
  ...
  adminStatsContent.appendChild(backupLine);

  // Per-user breakdown heading
  const heading = document.createElement("div");
  heading.style.cssText = "font-weight:700; font-size:0.9rem; margin-bottom:8px;";
  heading.textContent = "Per-User Breakdown";
  adminStatsContent.appendChild(heading);
  ...
  adminStatsContent.appendChild(tableWrap);
};
...
const loadAdminStats = async () => {
  ...
  adminStatsContent.appendChild(loading);
  ...
  try {
    ...
    adminStatsContent.appendChild(errEl);
  }
};
...
const initAdminLink = async () => {
  ...
  adminAnchor.addEventListener("click", (event) => {
    event.preventDefault();
    loadAdminStats();
    openDialog(adminDialog);
    resetDialogScroll(adminDialog);
  });
